...
% Tests 2 dimensional case of hac.fpdf with copulapdf
    U = csvread('data/data2d.csv');
    assertVectorsAlmostEqual(hac.fastPdf('clayton', U, { 1 2 1.5 }), copulapdf('clayton', U, 1.5));
    assertVectorsAlmostEqual(hac.fastPdf('frank', U,  { 1 2 1.5 }), copulapdf('frank', U, 1.5));
    assertVectorsAlmostEqual(hac.fastPdf('gumbel', U, { 1 2 1.5 }), copulapdf('gumbel', U, 1.5));
    
function testHacFastPdfFlat3D
% Tests 2 dimensional case of hac.fpdf with copulapdf
    U = csvread('data/data3d.csv');
    assertVectorsAlmostEqual(hac.fastPdf('clayton', U, { 1 2 3 1.5 }), archim.pdf('clayton', U, 1.5));
    assertVectorsAlmostEqual(hac.fastPdf('frank', U,  { 1 2 3 1.5 }), archim.pdf('frank', U, 1.5));
    assertVectorsAlmostEqual(hac.fastPdf('gumbel', U, { 1 2 3 1.5 }), archim.pdf('gumbel', U, 1.5));
    
function testHacFastPdf3DAgainsHacPdf
    U = csvread('data/data3d.csv');
    tree =  {{1 2 2.0} 3 1.5};    
    assertVectorsAlmostEqual(hac.pdf('clayton', U, tree), hac.fastPdf('clayton', U, tree));
    assertVectorsAlmostEqual(hac.pdf('gumbel', U, tree), hac.fastPdf('gumbel', U, tree));
    assertVectorsAlmostEqual(hac.pdf('frank', U, tree), hac.fastPdf('frank', U, tree));
